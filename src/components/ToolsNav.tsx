...
import { Wand2, QrCode, FileText, Receipt, Palette, Smartphone, Stamp, FileImage, Timer, Calculator } from 'lucide-react';
      ...
      <div className="flex overflow-x-auto scrollbar-hide gap-1.5 bg-slate-900/50 p-1.5 rounded-2xl backdrop-blur-md border border-slate-800 max-w-full">
        {links.map((link) => {
          const isActive =
            ...
            (link.path === '/outils/detourage' && currentPath === '/outils');
          return (
            <Link
              key={link.path}
              to={link.path}
