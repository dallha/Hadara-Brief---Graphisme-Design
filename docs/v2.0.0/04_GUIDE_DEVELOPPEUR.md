...
*   `components/` : Contient l'ensemble des composants d'interface.
    ...
    *   `AdminDashboard.tsx` : Point d'entrée de l'administration frontend. Il a été simplifié pour ne proposer qu'une vue **Kanban en lecture seule** (`KanbanTab.tsx`). Toute la logique de modification métier complexe (CRM, édition de prix, etc.) est déléguée à Django Jazzmin pour éviter la duplication de logique.
